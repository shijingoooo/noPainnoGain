...
public class FetchVideoFrameUtil {
    ...
    public static void main(String[] args){
        try {
            String picPath = "/Users/shijing05/Desktop/work/城管小程序/静态文件/pic/";  // 提取得每帧图片存放位置
//            String videoPath = "/Users/shijing05/Desktop/work/城管小程序/静态文件/city-admin-promotion.mp4";  // 原视频文件路径
            String videoPath = "/Users/shijing05/Downloads/open_video.m3u";  // 原视频文件路径
            int second = 0; // 每隔多少帧取一张图，一般高清视频每秒 20-24 帧，根据情况配置，如果全部提取，则将second设为 0 即可
            // 开始视频取帧流程
            FetchVideoFrameUtil.fetchPic(picPath,second);
     ...
     * @throws Exception
     */
    public static void fetchPic(String picPath,int second) throws Exception{

//        FFmpegFrameGrabber ff = new FFmpegFrameGrabber(file); // 获取视频文件
        String streamUrl = "rtmp://10.175.100.156:8935/live/s";
        FFmpegFrameGrabber ff = new FFmpegFrameGrabber(streamUrl);
        ff.setOption("rw_timeout", "60000000");
//        System.out.println(FetchVideoFrameUtil.getVideoTime(file)); // 显示视频长度（秒/s）

//        ff.start(); // 调用视频文件播放
        ff.startUnsafe();
        int length = ff.getLengthInVideoFrames(); //视频帧数长度
        ...
        int count = 0;
        while (i < 10000) {
//            frame = ff.grabKeyFrame(); // 获取该帧图片流
            frame = ff.grabKeyFrame();
